...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python verbose_similarity_demo.py <file1> <file2>")
        sys.exit(1)
    ...
    print("\n" + "="*60)
    print(" 🚀 INTERACTIVE CKG SIMILARITY DEMO (VERBOSE MODE)")
    print("="*60)
    ...
    # ADDED: Print code contents
    print("="*60)
    print(f" 📄 FILE 1 CONTENT ({os.path.basename(f1)}):")
    print("="*60)
    with open(f1, 'r', encoding='utf-8', errors='ignore') as f:
        print(f.read().strip())
        
    print("\n" + "="*60)
    print(f" 📄 FILE 2 CONTENT ({os.path.basename(f2)}):")
    print("="*60)
