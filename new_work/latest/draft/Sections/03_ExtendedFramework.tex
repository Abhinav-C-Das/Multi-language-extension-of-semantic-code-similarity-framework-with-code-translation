...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{Sections/architecture1.png}
...
In Phase~1, we established a three-view CPG-based framework for measuring algorithmic similarity in single-language C educational assignments, achieving 88.5\% top-1 retrieval accuracy on 104 C programs. The framework combines: (i) a \textit{Baseline view} computing TF-IDF similarity over normalized code identifiers; (ii) a \textit{Structural view} computing Weisfeiler-Lehman graph kernel similarity over CPG AST nodes; and (iii) a \textit{Semantic view} based on Computation Evolution Signatures (CES). These three views are combined using a linear weighted late fusion with the weights set to $(w_{\text{BL}}, w_{\text{WL}}, w_{\text{CES}}) = (0.35, 0.40, 0.25)$, tuned by exhaustive grid search (in 0.05 increment steps) over a dataset of 104 C-language submissions collected across the same 20-problem institutional corpus as evaluated in Phase~2. The optimal WL graph kernel depth $h=2$ was validated to capture local control-flow context without hash fragmentation across subgraph boundaries (as ablated in Table~\ref{tab:wl_ablation}). The Phase~1 semantic view relied on a baseline taxonomy of 9 core CES patterns (explicitly enumerated with the ``Ph.~1 \& 2'' designation in Table~\ref{tab:ces_taxonomy}). The present manuscript extends this self-contained Phase~1 foundation to multi-lingual evaluation across C, C++, and Java.
