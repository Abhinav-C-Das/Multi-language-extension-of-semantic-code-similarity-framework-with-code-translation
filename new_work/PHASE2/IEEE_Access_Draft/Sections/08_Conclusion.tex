...
Empirically, we find that the aggregate cross-language retrieval accuracy is 87.25\% ($n=400$), with the Java$\rightarrow$C/C++ direction exhibiting 91.43\% top-1 recall ($n=140$). This aggregate performance is stratified by directional asymmetry (detailed in Section~\ref{subsec:cross_lang_results}), which serves as the primary boundary condition of our empirical results. The feature weights, trained only using monolingual C samples, prove invariant throughout our mixed-language evaluations. When compared against a fine-tuned UniXcoder model in a setting that ensures an apples-to-apples comparison, the deterministic method exhibits a 17.38~percentage point (pp) improvement over the UniXcoder model for this low-resource setting but not necessarily under any arbitrary condition. In particular, compared against an independently optimized TF-IDF baseline, the framework provides a statistically significant 7.75~pp benefit (McNemar $p{<}0.0001$, OR$=4.875$ [95\% CI: 2.26, 10.52]), indicating the added value of both structural and semantic perspectives.
\textcolor{blue}{Four boundary conditions are outlined for the extent within which these results apply: first, the directional asymmetry of the parser frontend as noted above; second, the single institution data set limiting generalization across other contexts; third, the small number of references per problem in the cross-language reference pool (6--7 per query); and fourth, the limited applicability of the CES taxonomy only to CS-1 imperative patterns.} \textcolor{blue}{The APM generator is merely a proof-of-feasibility under author-constructed test conditions; the agreement percentage of 98.3\% should therefore be seen strictly as a consistency measure and not an established performance benchmark.} All these limitations are discussed further in Section~\ref{subsec:threats}.
...
\begin{enumerate}
    ...
    \item \textcolor{blue}{Using fuzz tests from third parties on the rule generation engine using an extended CS-1 problem corpus and external test suites for determining failure modes.}
...
\url{https://github.com/Abhinav-C-Das/ckg-multiview-code-similarity}
