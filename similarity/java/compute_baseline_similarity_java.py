...
OUT_FILE = Path("evaluation/java/similarity_matrix_java.json")

print("[Baseline Similarity] Loading baseline features...")
...
# Scan for baseline features (FIXED: look for combined_features.json)
problems = {}
...
print(f"[Baseline Similarity] Found {len(problems)} problems")
...
print(f"[Baseline Similarity] ✓ Saved similarity matrix to {OUT_FILE}")
print(f"[Baseline Similarity] Total comparisons: {sum(len(students) for students in results.values())}")
